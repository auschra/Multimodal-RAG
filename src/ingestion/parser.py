...
def main():
    ...
    pdfs = sorted(f for f in corpus_dir.iterdir() if f.suffix.lower() == ".pdf")            # sorted list of pdf filenames 
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:                  
        ...
        jobs = {executor.submit(parse_file, pdf, client): pdf for pdf in pdfs}              # create queue of parsing jobs

        for i, job in enumerate(concurrent.futures.as_completed(jobs), 1):              
            pdf = jobs[job]
            
            try:
                job.result()                                                                # output written as json
