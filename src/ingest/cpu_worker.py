...
"""
CPU worker will the following tasks:
    - parse PDF with docling, chunk using document structural tags and token considerations
    - standardise the coord system for chunk bounding boxes
    - extract document images for GPU colpali worker to embed
"""
def docling_worker(pdf_queue, image_queue):
    ...
    chunker = HybridChunker(tokenizer=tokenizer, max_tokens=512)                                                # Unsure on max_tokens and whether hybridchunker is the right choice
    ...
    while True:
        ...
        stem = Path(pdf_path).stem                                                                      # PDF name for doc tracking later

        # Convert PDF to docling document & create markdown file
        ...
        for i, chunk in enumerate(chunker.chunk(result)):
            ...
            heading = chunk.meta.headings[0] if chunk.meta.headings else "Root"                         # Extract heading if present         
            
            chunk_type = "text"                                                                         # Default back to text
            parent = [] 
